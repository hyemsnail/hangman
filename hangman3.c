#include <stdio.h>
#include <Windows.h>
#include <time.h> // 랜덤 값 가져오기 위해 사용
#include <stdlib.h>
...
void init() {
    ...
    system("mode con cols= 70 lines=70 | title Hangman Game");//콘솔창 크기 조절(폭 94문자, 높이 25줄), 콘솔창 이름 설정
...
void game() {
    ...
    while (1) {
        ...
        printf("\n진행 상황: %s\n", answer[random]);
        printf("문자 입력: ");
        ch = getchar(); // getchar(): 입력한 문자 버퍼o ,echo 즉 엔터 쳐야 전달
        if (check(solution[random], answer[random], ch) == 1) { //1: 모든 문자 일치 정답! => 게임 종료 0: 이어서 입력 받기
            printf("정답! %s\n", answer[random]);
            break;
...
int check(char sol[], char ans[], char ch) { //solution[random][], answer[random][], ch
    int tmp = 0;
    for (int i = 0; sol[i] != 0; i++) {
        if (sol[i] == ch) {//soution에 입력한 문자와 일치하는 것이 있다면
            ans[i] = ch; //answer[random]의 해당 위치에 문자 넣어주기(ex. ___ -> __a_)
            tmp = 1;
        }
    }
